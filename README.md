# Cybersecurity Portfolio
Welcome! I’m building hands-on skills in cybersecurity — focusing on threat analysis, SIEM monitoring, incident response, and defensive security — using TryHackMe Premium and personal labs.

## 📂 Projects
| # | Project Name | Focus Area | Status |
|---|---|---|---|
| 1 | Password Security Analysis | Authentication & Risk Assessment | ✅ Complete |
| 2 | Phishing Email Analysis | Threat Detection & Forensics | ✅ Complete |
| 3 | Vulnversity: Web Vulnerability Assessment | Web App Security | 🚧 In Progress |
| 4 | Splunk SIEM Lab: Brute-Force Detection | Log Analysis & Monitoring | ⏳ Planned |
| 5 | Blue Team Incident Response Walkthrough | Defence & IR | ⏳ Planned |

## 📜 Certifications
- **Google Cybersecurity Professional Certificate** — Completed [January 2026]
  Covered: threat analysis, network security, risk management, SIEM & incident response basics

- **CompTIA A+** — 📖 In Progress | Exam scheduled: November 2026
  Building core hardware, networking, and system administration fundamentals — essential for any cybersecurity role
  
## 🛠️ Tools & Technologies
- **Platforms:** TryHackMe (Premium)
- **Tools:** nmap, Splunk, Wireshark, Linux (Kali/ Ubuntu), basic Python
- **Frameworks:** MITRE ATT&CK, NIST SP 800-61 Incident Response
- **Core Skills:** Threat analysis, IOC identification, log analysis, vulnerability assessment

## 📚 Current Learning Path
- TryHackMe → **SOC Level 1** Path
- Focus: Defensive security, SIEM, incident investigation

## 📫 Connect
- **GitHub:** https://github.com/stephanieorogun
- **LinkedIn:** https://linkedin.com/in/stephanie-orogun-978099166
- **Email:** enemmohchiamaka@gmail.com
- **TryHackMe:** [https://tryhackme.com/p/enemmohchiamaka] 🟢 Active — SOC Level 1 Path
